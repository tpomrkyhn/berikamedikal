    ...
    <div class="page-content">
        ...
        <?
            ...
            if(!isset($_GET["s"])){
                ...
                go("index.php?do=mailyildiz&s=1");

            }

            $sayfa = g("s");

            $sayfaAdet = ceil(mailyildiz() / $maxAdet);
            ...
            if($sayfa > $sayfaAdet){
                ...
                go("index.php?do=mailyildiz&s=".$sayfaAdet);
        ...
        <div class="content-wrapper">
            ...
            <div class="panel panel-white">
                <div class="panel-heading">
                    <h6 class="panel-title"><?= $d["yildizlilar"] ?></h6>
                </div>
                    ...
                    <div class="navbar navbar-default">
                        ...
                        <div class="navbar-collapse collapse" id="inbox-toolbar-toggle-single">
                            ...
                            <div class="navbar-right">
                                <p class="navbar-text"><span class="text-semibold"><?= $sayfaBaslangic ?> - <? if($sayfaBaslangic + $maxAdet > mailyildiz()){ echo mailyildiz(); }else{ echo $sayfaBaslangic + $maxAdet; } ?></span> / <span class="text-semibold"><?= mailyildiz() ?></span></p>

                                <div class="btn-group navbar-left navbar-btn">
                                    <button <? if($sayfa > 1){ ?> onclick="window.location.href = 'index.php?do=mailyildiz&s=<?= $sayfa - 1 ?>';" <? } ?> type="button" class="btn btn-default btn-icon <? if($sayfa == 1){ echo "disabled"; } ?>"><i class="icon-arrow-left12"></i></button>
                                    <button <? if($sayfa < $sayfaAdet){ ?> onclick="window.location.href = 'index.php?do=mailyildiz&s=<?= $sayfa + 1 ?>';" <? } ?> type="button" class="btn btn-default btn-icon <? if($sayfa == $sayfaAdet){ echo "disabled"; } ?>"><i class="icon-arrow-right13"></i></button>
                                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-inbox">
                            ...
                            function yildiz(id) {
                                ...
                                $.ajax({
                                    ...
                                    success:function(e){ // gonderme işlemi başarılı ise e değişkeni ile gelen değerleri aldı
                                        if(e == "dolu"){}else if(e == "bos"){ $("#mail_"+id).hide(); adeteksilt(); }
                                    }
                                });
                        ...
                        $yildizlar = query("SELECT * FROM yildizlar WHERE yildiz_tip = 'mail' AND yildiz_uye_id = '".session("uye_id")."' ORDER BY yildiz_id DESC LIMIT $sayfaBaslangic,$maxAdet");
                        while ($yildizz = row($yildizlar)){

                            $mailler = query("SELECT * FROM mailler WHERE mail_id = '".$yildizz["yildiz"]."'");
                            $mail = row($mailler);
                                ...
                                $yildiz = "evet"; $yildizsinif = "icon-star-full2 text-orange";
