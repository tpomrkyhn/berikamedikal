    ...
    <div class="page-content">
        ...
        <?
            ...
            if(!isset($_GET["s"])){
                ...
                go("index.php?do=mailgiden&s=1");

            }

            $sayfa = g("s");

            $sayfaAdet = ceil(mailgiden() / $maxAdet);
            ...
            if($sayfa > $sayfaAdet){
                ...
                go("index.php?do=mailgiden&s=".$sayfaAdet);
        ...
        <div class="content-wrapper">
            ...
            <div class="panel panel-white">
                <div class="panel-heading">
                    <h6 class="panel-title"><?= $d["gidenkutusu"] ?></h6>
                </div>
                    ...
                    <div class="navbar navbar-default">
                        ...
                        <div class="navbar-collapse collapse" id="inbox-toolbar-toggle-single">
                            ...
                            <div class="navbar-right">
                                <p class="navbar-text"><span class="text-semibold"><?= $sayfaBaslangic ?> - <? if($sayfaBaslangic + $maxAdet > mailgiden()){ echo mailgiden(); }else{ echo $sayfaBaslangic + $maxAdet; } ?></span> / <span class="text-semibold"><?= mailgiden() ?></span></p>

                                <div class="btn-group navbar-left navbar-btn">
                                    <button <? if($sayfa > 1){ ?> onclick="window.location.href = 'index.php?do=mailgiden&s=<?= $sayfa - 1 ?>';" <? } ?> type="button" class="btn btn-default btn-icon <? if($sayfa == 1){ echo "disabled"; } ?>"><i class="icon-arrow-left12"></i></button>
                                    <button <? if($sayfa < $sayfaAdet){ ?> onclick="window.location.href = 'index.php?do=mailgiden&s=<?= $sayfa + 1 ?>';" <? } ?> type="button" class="btn btn-default btn-icon <? if($sayfa == $sayfaAdet){ echo "disabled"; } ?>"><i class="icon-arrow-right13"></i></button>
                                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-inbox">
                        ...
                        <?
                            ...
                            $mailler = query("SELECT * FROM mailler WHERE mail_gonderen = '".session("uye_id")."' ORDER BY mail_id DESC LIMIT $sayfaBaslangic,$maxAdet");
                            while ($mail = row($mailler)){
                                ...
                                $uye = query("SELECT * FROM uyeler WHERE uye_id = '".$mail["mail_alan"]."'");
                                $uye = row($uye);
                        ...
                        <tr id="mail_<?= $mail["mail_id"] ?>" class="read">
